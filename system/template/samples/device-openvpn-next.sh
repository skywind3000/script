...
if [ "$1" = "up" ]; then
	...
	/etc/script/bin/route-next-install.sh $INDEX $DEV $GW $VPN_SERVER $VPN_DEV $VPN_GW $VPN_SUBNET
	...
	/etc/script/bin/route-next-remove.sh $INDEX $DEV $GW $VPN_SERVER $VPN_DEV $VPN_GW $VPN_SUBNET
fi
