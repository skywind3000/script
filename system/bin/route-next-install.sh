...
ip route add table 700 $VPN_SERVER via $GATEWAY dev $DEVICE

# vpn sub network
if [ -n "$VPN_SUBNET" ]; then
	ip route add table 700 $VPN_SUBNET dev $VPN_DEVICE
fi
...
ip route add table $NAME default via $VPN_GATEWAY dev $VPN_DEVICE
ip rule add prio $NAME fwmark $NAME lookup $NAME
